...
$unread_msgs = 0;
$result      = $conn->query("SELECT COUNT(*) as c FROM contact_messages WHERE is_read = 0");
if ($result) $unread_msgs = $result->fetch_assoc()['c'];

$replied_msgs = 0;
$result       = $conn->query("SELECT COUNT(*) as c FROM contact_messages WHERE is_replied = 1");
if ($result) $replied_msgs = $result->fetch_assoc()['c'];

$active_services = 0;
$result          = $conn->query("SELECT COUNT(*) as c FROM services WHERE is_active = 1");
if ($result) $active_services = $result->fetch_assoc()['c'];

$active_clients = 0;
$result         = $conn->query("SELECT COUNT(*) as c FROM clients WHERE is_active = 1");
if ($result) $active_clients = $result->fetch_assoc()['c'];

$active_supplies = 0;
$result          = $conn->query("SELECT COUNT(*) as c FROM supplies WHERE is_active = 1");
if ($result) $active_supplies = $result->fetch_assoc()['c'];

$active_updates = 0;
$result         = $conn->query("SELECT COUNT(*) as c FROM updates WHERE is_active = 1");
if ($result) $active_updates = $result->fetch_assoc()['c'];

$supply_categories_count = 0;
$result                  = $conn->query("SELECT COUNT(*) as c FROM supply_categories WHERE is_active = 1");
if ($result) $supply_categories_count = $result->fetch_assoc()['c'];

// Recent messages (last 5)
$result       = $conn->query("SELECT * FROM contact_messages ORDER BY created_at DESC LIMIT 5");
$recent_msgs  = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
?>
...
<div class="ov-stat-grid">
    ...
    <div class="ov-stat-card">
        ...
        <div class="ov-stat-icon" style="background:rgba(21,101,192,.1);color:var(--primary-color);">
    ...
    <div class="ov-stat-card">
        ...
        <div class="ov-stat-icon" style="background:rgba(0,123,255,.1);color:#007BFF;">
    ...
    <div class="ov-stat-card">
        ...
        <div class="ov-stat-icon" style="background:rgba(230,81,0,.1);color:#E65100;">
    ...
    <div class="ov-stat-card">
        ...
        <div class="ov-stat-icon" style="background:rgba(106,27,154,.1);color:#6A1B9A;">
    ...
    <div class="ov-stat-card">
        ...
        <div class="ov-stat-icon" style="background:rgba(40,167,69,.1);color:#28A745;">
    ...
    <div class="ov-stat-card" style="<?php echo $stats['unread_messages'] > 0 ? 'border-color:#FFC107;' : ''; ?>">
        ...
        <div class="ov-stat-icon" style="background:rgba(255,193,7,.15);color:#FFC107;">
...
<div class="ov-two-col">
    ...
    <div class="admin-card" style="margin-bottom:0;">
        ...
        <div class="ov-quick-grid">
            ...
            <a href="dashboard.php?page=messages" class="ov-quick-btn"
               style="<?php echo $stats['unread_messages'] > 0 ? 'border-color:#FFC107;color:#856404;' : ''; ?>">
        ...
        <div class="ov-section-title" style="margin-top:1.5rem;">
            <i class="fas fa-layer-group"></i> Content Summary
        </div>

        <?php
        $modules = [
            ['label' => 'Clients',          'total' => $stats['total_clients'],  'active' => $active_clients,           'icon' => 'fas fa-users',     'color' => 'var(--primary-color)'],
            ['label' => 'Services',          'total' => $stats['total_services'], 'active' => $active_services,          'icon' => 'fas fa-cogs',      'color' => '#007BFF'],
            ['label' => 'Supply Categories', 'total' => $supply_categories_count, 'active' => $supply_categories_count,  'icon' => 'fas fa-tag',       'color' => '#6A1B9A'],
            ['label' => 'Supplies',          'total' => $stats['total_supplies'], 'active' => $active_supplies,          'icon' => 'fas fa-boxes',     'color' => '#E65100'],
            ['label' => 'Posts / Updates',   'total' => $stats['total_updates'],  'active' => $active_updates,           'icon' => 'fas fa-newspaper', 'color' => '#6A1B9A'],
        ];
        ?>
            ...
            <?php foreach ($modules as $m): ?>
                <tr style="border-bottom:1px solid var(--border-color);">
                    <td style="padding:.6rem 0;">
                        <i class="<?php echo $m['icon']; ?>" style="color:<?php echo $m['color']; ?>; margin-right:.5rem; width:16px; text-align:center;"></i>
                        <?php echo $m['label']; ?>
                    </td>
                    <td style="text-align:center; padding:.6rem .5rem; font-weight:700;">
                        <?php echo $m['total']; ?>
                    </td>
                    <td style="text-align:right; padding:.6rem 0;">
                        <span style="background:rgba(40,167,69,.12); color:#28A745; border-radius:50px; padding:.15rem .55rem; font-size:.72rem; font-weight:800;">
                            <?php echo $m['active']; ?> active
                        </span>
                    </td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div><!-- /.admin-card (quick actions) -->

    <!-- Recent Messages -->
    <div class="admin-card" style="margin-bottom:0;">
        <div class="ov-section-title" style="justify-content:space-between;">
            <span><i class="fas fa-envelope"></i> Recent Messages</span>
            <a href="dashboard.php?page=messages" style="font-size:.78rem; font-weight:700; color:var(--primary-color); text-decoration:none;">
                View all →
            </a>
        </div>
        ...
        <?php if (!empty($recent_msgs)): ?>
                    ...
                    ?>
                        <tr>
                            <td style="font-weight:600;">
                                <?php echo sanitize($msg['full_name']); ?>
                                <br>
                                <span style="font-size:.75rem; color:var(--text-light); font-weight:400;">
                                    <?php echo sanitize($msg['email']); ?>
                                </span>
                            </td>
                            <td style="font-size:.8rem; color:var(--text-light);">
                                <?php echo sanitize($msg['service_needed'] ?? '—'); ?>
                            </td>
                            <td style="font-size:.78rem; color:var(--text-light); white-space:nowrap;">
                                <?php echo date('M j', strtotime($msg['created_at'])); ?>
                            </td>
                            <td>
                                <span class="ov-badge <?php echo $sk; ?>">
            ...
            <?php if ($stats['total_messages'] > 5): ?>
                <div style="text-align:center; padding-top:.75rem; font-size:.8rem; color:var(--text-light);">
                    ...
                    <a href="dashboard.php?page=messages" style="color:var(--primary-color); font-weight:700; text-decoration:none;">
                        View all
                    </a>
                </div>
            <?php endif; ?>
        ...
        <?php else: ?>
            <p style="text-align:center; color:var(--text-light); padding:2rem 1rem;">
                <i class="fas fa-inbox"></i> No messages yet.
            </p>
        <?php endif; ?>
    </div><!-- /.admin-card (recent messages) -->
